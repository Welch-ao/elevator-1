...
{
    Elevator *ele = static_cast(e.GetSender());
...
{
    Person *person = static_cast(e.GetSender());
...
{
    Person *person = static_cast(e.GetSender());
    Elevator *ele = static_cast(e.GetEventHandler());
...
{
    Person *person = static_cast(e.GetSender());
    Interface *interf = static_cast(e.GetEventHandler());
...
{
    std::chrono::steady_clock::time_point now = std::chrono::steady_clock::now(); std::chrono::duration elapsed = std::chrono::duration_cast>(now - start); if (elapsed.count() > 120) throw std::runtime_error("Test run exceeded its maximum run time of 120 seconds."); for (auto pair : deadlines_)
    {
